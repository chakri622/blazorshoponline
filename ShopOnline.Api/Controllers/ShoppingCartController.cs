    ...
    public class ShoppingCartController : ControllerBase
    {
        ...
        public async Task<ActionResult<CartItemDto>> PostItem([FromBody] CartItemToAddDto cartItemToAddDto)
        {
            try
            {
                ...
                var product = await this.productRepository.GetProduct(newCartItem.ProductId);
                if (product == null)
                {
                    throw new Exception($"Something went wrong when attempting to determine product (ProductId:({cartItemToAddDto.ProductId})) for the cartItem added");
                }
